...
def sendCodeSnapshot():
        ...
        for window in sublime.windows():
            for view in window.views():
                ...
                if filename in gemsActiveFiles:
                    ...
                    if problem_id in lastSentCodes and lastSentCodes[problem_id] == code:
                        continue
                    data = {
                        'code': code,
    ...
    if gemsSnapshotTracking:
        sublime.set_timeout_async(sendCodeSnapshot, 15000)
...
def updateActiveProblems():
    ...
    try:
        ...
        for problem in response['ActiveProblems']:
            gemsActiveFiles[os.path.join(
                gemsFOLDER, problem['Filename'])] = problem['ProblemID']
    except:
        gemsSnapshotTracking = False
    if gemsSnapshotTracking:
...
class gemsNeedHelp(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        sublime.active_window().show_input_panel("Click Enter to send help request:",
                                                 "", self.get_need_help_with, None, self.get_need_help_with)
                ...
                if os.path.exists(local_file):
                    with open(local_file) as f:
                        ...
                        with open(moved_file, 'w', encoding='utf-8') as newf:
                            newf.write(f.read())
                    mesg = 'Move existing file, {}, to {}.'.format(
                        filename, old_dir)
...
def gemsRequest(path, data, authenticated=True, method='POST', verbal=True):
    ...
    if 'Folder' not in info:
        if verbal:
            sublime.message_dialog(
                "Please set a local folder to store working files.")
        return None
...
class gemsAddTestCase(sublime_plugin.ApplicationCommand):
    ...
    def set_output(self, expected_output):
        ...
        if len(expected_output) > 0:
            ...
            testCaseFile = os.path.join(os.path.dirname(
                os.path.realpath(__file__)), testCaseFile)
            try:
                ...
                test_cases = []
            test_cases.append(
                {'input': self.test_input, 'output': expected_output})
