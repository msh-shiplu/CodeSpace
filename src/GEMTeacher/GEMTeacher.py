        ...
        if gemtSERVER == '':
            ...
            if gemtSERVER == '':
                sublime.message_dialog(
                    'Unable to connect. Check server address.')
                return
...
def gemt_get_problem_info(fname):
    ...
    if first_line.startswith('#'):
        ...
        first_line = first_line.strip('/ ')
    try:
        items = re.match('(\d+)\s+(\d+)\s+(\d+)(\s+(\w.*))?',
                         first_line).groups()
        merit, effort, attempts, tag = int(items[0]), int(
            items[1]), int(items[2]), items[4]
        if tag is None:
            tag = ''
        tag = tag.strip()
...
class gemtShare(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        if fname is None:
            ...
            return
        content, answer, merit, effort, attempts, tag, name, exact_answer = gemt_get_problem_info(
            fname)
        data = {
            'content': 		content,
            'answer':		answer,
...
def gemtRequest(path, data, authenticated=True, method='POST'):
    ...
    if 'Folder' not in info:
        sublime.message_dialog(
            "Please set a local folder for keeping working files.")
        return None
    ...
    if authenticated:
        if 'Name' not in info or 'Password' not in info:
            sublime.message_dialog(
                "Please ask to setup a new teacher account and register.")
            return None
        data['name'] = info['Name']
...
class gemtViewExercises(sublime_plugin.ApplicationCommand):
    def run(self):
        global gemtSERVER
        with open(gemtFILE, 'r') as f:
            info = json.loads(f.read())
        ...
        webbrowser.open(gemtSERVER + '/view_exercises?'+p)
...
class gemtAddBulletin(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        if len(content) <= 20:
            sublime.message_dialog(
                'Select more text to show on the bulletin board.')
            return
        response = gemtRequest('teacher_adds_bulletin_page', {
                               'content': content})
        if response:
            sublime.message_dialog(response)
...
def gemt_gets(self, index, priority):
    ...
    response = gemtRequest(
        'teacher_gets', {'index': index, 'priority': priority})
    if response is not None:
        ...
        if len(folder) > 0:
            ...
            if not os.path.exists(folder):
                try:
                    ...
                    sublime.message_dialog(
                        'Could not create {}.'.format(folder))
            else:
                with open(gemtFILE, 'w') as f:
                    f.write(json.dumps(info, indent=4))
                sublime.message_dialog(
...
class gemtConnect(sublime_plugin.ApplicationCommand):
    def run(self):
        ...
        load = urllib.parse.urlencode(
            {'who': info['CourseId']}).encode('utf-8')
...
class gemtCompleteRegistration(sublime_plugin.ApplicationCommand):
    def run(self):
        ...
        response = gemtRequest(
            ...
            {'role': 'teacher', 'name': info['Name'],
                'course_id': info['CourseId']},
            authenticated=False,
        )
        if response is None:
            sublime.message_dialog(
                'Response is None. Failed to complete registration.')
            return
    ...
    def run(self):
        ...
        if sublime.ok_cancel_dialog("Current version is {}. Click OK to update.".format(version)):
            ...
            urllib.request.urlretrieve(
                "https://raw.githubusercontent.com/vtphan/GEM/alina/src/GEMTeacher/GEMTeacher.py", module_file)
            urllib.request.urlretrieve(
                "https://raw.githubusercontent.com/vtphan/GEM/alina/src/GEMTeacher/Main.sublime-menu", menu_file)
            urllib.request.urlretrieve(
                "https://raw.githubusercontent.com/vtphan/GEM/alina/src/GEMTeacher/Default.sublime-keymap", keymap_file)
            urllib.request.urlretrieve(
            ...
            with open(os.path.join(package_path, "VERSION"), 'w') as f:
                f.write(version)
            sublime.message_dialog(
                "GEM has been updated to version %s." % version)
