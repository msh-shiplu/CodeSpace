...
func create_tables() {
	...
	execSQL("create table if not exists snapshot_feedback (id integer primary key, snapshot_id integer, feedback text, author_id integer, author_role string, given_at timestamp, is_helpful string, updated_at timestamp)")
...
func init_database(db_name string) {
	...
	UpdateSnapshotFeedbackSQL = prepare("update snapshot_feedback set is_helpful=?, updated_at=? where id=?")
