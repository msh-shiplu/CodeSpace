...
func create_tables() {
	...
	execSQL("create table if not exists submission (id integer primary key, problem_id integer, student_id integer, student_code blob, snapshot_id integer default 0, submission_category integer, code_submitted_at timestamp, completed timestamp, verdict text, attempt_number integer)")
...
func init_database(db_name string) {
	...
	AddSubmissionSQL = prepare("insert into submission (problem_id, student_id, student_code, submission_category, attempt_number, code_submitted_at, snapshot_id) values (?, ?, ?, ?, ?, ?, ?)")
	AddSubmissionCompleteSQL = prepare("insert into submission (problem_id, student_id, student_code, submission_category, attempt_number, code_submitted_at, completed, snapshot_id) values (?, ?, ?, ?, ?, ?, ?, ?)")
